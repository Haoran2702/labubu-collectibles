...
export default function ForecastingPage() {
  ...
  const fetchForecasts = async () => {
    try {
      const token = sessionStorage.getItem("admin_jwt");
      const response = await fetch('http://localhost:3001/api/forecasting/dashboard', {
        headers: {
  ...
  const generateForecast = async () => {
    ...
    try {
      const token = sessionStorage.getItem("admin_jwt");
      const response = await fetch(`http://localhost:3001/api/forecasting/generate/${selectedProduct}`, {
        method: 'POST',
        headers: {
