...
export default function AdminTicketDetailsPage() {
  ...
  async function fetchTicket() {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:3001/api/support/${ticketId}`, {
  ...
  async function handleReply(e: React.FormEvent) {
    ...
    try {
      const token = sessionStorage.getItem("admin_jwt");
      const res = await fetch(`http://localhost:3001/api/support/${ticketId}/reply`, {
        method: "POST",
        headers: {
  ...
  async function handleStatusChange(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    try {
      const token = sessionStorage.getItem("admin_jwt");
      const res = await fetch(`http://localhost:3001/api/support/${ticketId}/status`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
