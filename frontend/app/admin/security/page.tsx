...
export default function SecurityPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      // Fetch fraud stats
      const fraudRes = await fetch("http://localhost:3001/api/fraud/stats", {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (fraudRes.ok) {
      ...
      // Fetch GDPR stats
      const gdprRes = await fetch("http://localhost:3001/api/privacy/admin/gdpr-stats", {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (gdprRes.ok) {
      ...
      const requestsRes = await fetch("http://localhost:3001/api/privacy/admin/data-rights", {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (requestsRes.ok) {
  ...
  async function processDataRightsRequest(requestId: string) {
    ...
    try {
      const token = sessionStorage.getItem("admin_jwt");
      const res = await fetch(`http://localhost:3001/api/privacy/admin/data-rights/${requestId}/process`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
