...
export default function ContactPage() {
  ...
  async function fetchTickets() {
    ...
    try {
      ...
      const ticketsWithReplies = await Promise.all(filtered.map(async (ticket: any) => {
        try {
          const resMsg = await fetch(`http://localhost:3001/api/support/${ticket.id}`, {
            headers: token ? { Authorization: `Bearer ${token}` } : {},
          });
  ...
  async function openChat(ticket: any) {
    ...
    try {
      const token = localStorage.getItem("authToken");
      const res = await fetch(`http://localhost:3001/api/support/${ticket.id}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
  ...
  async function handleReply(e: React.FormEvent) {
    ...
    try {
      const token = localStorage.getItem("authToken");
      const res = await fetch(`http://localhost:3001/api/support/${openChatTicket.id}/reply`, {
        method: "POST",
        headers: {
